...
use crate::accounts::{cast, cast_mut, Data};
use crate::config::LOCK_DURATION;
use crate::error::Error;
use crate::error::Error::CalculationUnderflow;
// #[repr(u8)]
// #[derive(Clone, Copy)]
// pub enum LockType {
//     Ro = 0,
...
impl Lock {
    pub fn init(info: &AccountInfo /*, typ: AccountType*/) -> Result<(), Error> {
        //Ver::init(info, typ)?;
    ...
    // pub fn is_managed(info: &AccountInfo, program_id: &Pubkey) -> Result<bool, Error> {
    ...
    pub fn get(&self) -> Result<bool, Error> {//Result<Option<LockType>, Error> {
        //let mut lock = None;
        let mut lock = false;
    ...
    //     Ok(())
    // }
    pub fn lock(&mut self, holder: &Pubkey) -> Result<(), Error> {
        self.lock = true;
        self.holder = holder.to_bytes();
        self.timestamp = Clock::get()?.unix_timestamp;
        Ok(())
    }
